...
LOG_FILE = "C:\\Users\\ianre\\Desktop\\coda\\xbee\\ian\\log.txt"
...
def CANComm():
    ...
    while running_flag:
        ...
        #global_rpm = int.from_bytes(int(random()*100), byteorder = BYTEORDER_CONSTANT, signed = False)
        #print("rand" + int(random()*100) + " global_rpm:"+ str(global_rpm))
    ...
    def updateScreen(self, *args):
        try:
            with open(LOG_FILE, 'r') as f:
                ...
                global_head_temperature = abs(round(float(split[0]),1))
                global_coolant_temperature = abs(round(float(split[1]),1))
                global_fuel_pressure = abs(round(float(split[2]),1))
                
                global_speed = abs(round(float(split[2]),1))
                global_rpm = (abs(round(float(split[2]),1))/200) * 5600  
        except Exception as e:
            print(e)

        #global_rpm = 3000 + int(random()*1000)
        #global_speed = 20 +  int(random()*1)
        #global_oil_pressure =  10 + int(random()*4)
        #global_battery_voltage = 14+ int(random()*5)

        #print("updateScreen: rand" + str(int(random()*100)) + " global_rpm:"+ str(global_rpm))
        if screens.current_screen == MainScreen:
